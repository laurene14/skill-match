...
export default class extends Controller {
  ...
  initCard(card) {
    ...
    hammertime.on('panend', (event) => {
      ...
      this.handleRemoveStatusClass(!keep);
      if (keep) {
  ...
  handleRemoveStatusClass(leave) {
    if (this.element.classList.contains('swipe--accept')) {
      this.element.classList.remove('swipe--accept');
      if (leave) {
        console.log("parti accept")
      }
    } else {
      this.element.classList.remove('swipe--refuse');
      if (leave) {
        console.log("parti refuse")
      }
    }
  }

  clickAccept(event) {
    this.handleClickStatusButton(1);
    event.preventDefault();
  }
  clickRefuse(event) {
    this.handleClickStatusButton(2)
    event.preventDefault();
  }

  clickFollow(event) {
    this.handleClickStatusButton(3)
    event.preventDefault();
  }

  handleClickStatusButton(status) {
    const activeCards = this.cardTargets.filter(card => !card.classList.contains("removed"));
